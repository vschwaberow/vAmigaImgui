...
void VirtualKeyboard::DrawKey(vamiga::VAmiga& emu, std::string_view label, int width,
                              int code) {
  if (ImGui::Button(label.data(), ImVec2(width == 0 ? 40 : width, 40))) {
    emu.keyboard.press((vamiga::KeyCode)code, 0.0, 0.05);
  }
}
